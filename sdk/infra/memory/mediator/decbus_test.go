...
func TestSubscribe(t *testing.T) {
	...
	b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("Hello There!"))
...
func TestSubscribeOnce(t *testing.T) {
	...
	b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("First Message"))
	b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("Second Message"))
...
func TestUnsubscribe(t *testing.T) {
	b := TransientDECBus()
	assert.NotNil(t, b)
	err := b.bus.SubscribeOnce(bogus.TestTopic, helloThere)
	assert.Nil(t, err)
	result := b.bus.HasCallback(bogus.TestTopic)
	assert.True(t, result)
...
func TestPublish(t *testing.T) {
	...
	assert.NotNil(t, b)
	result := ""
	err := b.bus.SubscribeOnce(bogus.TestTopic, func(msg *bogus.TestMsg) {
...
func TestSubscribeAsync(t *testing.T) {
	...
	assert.Nil(t, err)
	b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("hello"))
...
func TestSubscribeOnceAsync(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.True(t, b.bus.HasCallback(bogus.TestTopic))
	for i := 0; i < 11; i++ {
		time.Sleep(1 * time.Second)
		if i == 3 {
			b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("hello"))
		...
		fmt.Printf("On second %+v => HasCallback is %+v\n", i, b.bus.HasCallback(bogus.TestTopic))
	}
	assert.False(t, b.bus.HasCallback(bogus.TestTopic))
}

func TestWaitAsync(t *testing.T) {
	b := SingletonDECBus()
	assert.NotNil(t, b)
	err := b.bus.SubscribeAsync(bogus.TestTopic, longHelloThere, true)
	assert.Nil(t, err)
	b.bus.Publish(bogus.TestTopic, bogus.NewTestMsg("Testing WaitAsync"))
	b.WaitAsync()
}
